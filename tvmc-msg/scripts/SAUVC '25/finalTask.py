...
HEAVE_TARGET = 0.45 - HEAVE_TARGET_OFFSET
...
class FullDrive(StateMachine):
    ...
    trans_to_finished = nav_approach.to(finished)
    ...
    def on_enter_nav_search(self):
        ...
        for i in range(0,45,5):
            ...
            for j in range(0, len(detections), 5):
                ...
                if aspectRatio >0.2 and aspectRatio <0.6:
    ...
    def on_enter_nav_approach(self):
        ...
        c_time = time.time()
        # while (time.time() - c_time) <= NAV_SURGE_TIME:
